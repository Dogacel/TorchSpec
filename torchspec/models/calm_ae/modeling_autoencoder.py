# Vendored from https://github.com/shaochenze/calm (MIT License).
# CALM: Continuous Autoregressive Language Models — Shao et al., 2025.
# Unmodified except for this header and the relative import below.
# See THIRDPARTYNOTICES for the full license text.
...
from transformers.models.llama.modeling_llama import LlamaMLP, LlamaPreTrainedModel, LlamaRMSNorm
...
class Encoder(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder_layers = nn.ModuleList(
            [AELayer(config) for _ in range(config.num_encoder_layers)]
        )
    ...
    def forward(
        self, input_ids: torch.LongTensor = None, **kwargs
...
class Decoder(LlamaPreTrainedModel):
    def __init__(self, config):
        ...
        self.decoder_layers = nn.ModuleList(
            [AELayer(config) for _ in range(config.num_decoder_layers)]
        )
    ...
    def forward(self, latent_states, **kwargs) -> Union[Tuple, BaseModelOutputWithPast]:
...
class Autoencoder(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        latent_states = torch.nn.functional.dropout(
            latent_states, p=self.ae_dropout, training=self.training
        )
